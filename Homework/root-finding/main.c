...
void driver(void f(double t, gsl_vector* y, gsl_vector* dydt), double a, gsl_vector* ya, double b, gsl_vector* yb, double h, double acc, double eps, char* path);
...
void SE_solved(gsl_vector* x, gsl_vector* M){
	...
	driver(schroedinger, a, ya, b, yb, h, acc, eps, path);
...
int main(){
	...
	printf("Part B: solving and finding extremum of SE by solving the radial S.E equation.\n");
